...
#
if [ ! -d $PYENVDIR/lib/python2.6 ]; then
  cd $BUILDDIR
  ...
  # isolate system python from 32bit python2.6
  #
...
touch $PYENVNAME/.gitignore
echo "*" >  $PYENVNAME/.gitignore


# build pyclips with 32bit python
#
if [ ! -d $PYENVDIR/lib/python2.6/site-packages/clips ]; then
  cd $BUILDDIR
